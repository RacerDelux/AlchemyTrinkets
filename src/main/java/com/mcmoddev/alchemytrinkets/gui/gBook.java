...
import com.mcmoddev.alchemytrinkets.book.book;
import com.mcmoddev.alchemytrinkets.book.bookRegistry;
import com.mcmoddev.alchemytrinkets.book.page;
import com.mcmoddev.alchemytrinkets.util.RenderUtil;
...
public class gBook extends GuiScreen {
    String bookName = "";
    ...
    public gBook(String name, ItemStack book){
        this.bookName = name;
        this.bookItem = book;
    }
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton){
        ...
        if (page.type == com.mcmoddev.alchemytrinkets.book.page.EnumPageType.TABLE_OF_CONTENTS){
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks){
        ...
        for (int i = 0; i < book.pages.size(); i ++){
            if (book.pages.get(i).type == com.mcmoddev.alchemytrinkets.book.page.EnumPageType.TABLE_OF_CONTENTS){
                this.contentsPage = i;
            }
        }
        ...
        if (satisfiesReqs){
            RenderUtil.drawCenteredString(this.fontRenderer, I18n.format("magicpab.book."+book.name+"."+page.name+".title"), basePosX+96, basePosY+20, 0x2b2b2a);
        }
        else {
            RenderUtil.drawCenteredString(this.fontRenderer, I18n.format("magicpab.book.unknown_title"), basePosX+96, basePosY+20, 0x2b2b2a);
        }
        GlStateManager.color(1f, 1f, 1f, 1f);

        Minecraft.getMinecraft().getTextureManager().bindTexture(new ResourceLocation("roots:textures/gui/gui_book.png"));
        if (page.type == com.mcmoddev.alchemytrinkets.book.page.EnumPageType.TEXT && satisfiesReqs){
            List<String> strings = RenderUtil.getLines(I18n.format("magicpab.book."+book.name+"."+page.name+".desc"), 160);
            for (int i = 0; i < strings.size(); i ++){
                fontRenderer.drawString(strings.get(i), basePosX+20, basePosY+48+12*i, 0x2b2b2a);
                GlStateManager.color(1, 1, 1, 1);
            }
        }
        if (page.type == com.mcmoddev.alchemytrinkets.book.page.EnumPageType.DISPLAY && satisfiesReqs){
        ...
        if (page.type == com.mcmoddev.alchemytrinkets.book.page.EnumPageType.MORTAR && satisfiesReqs){
            ...
            if (book.selectedPage < book.pages.size()-1){
                ...
                for (int i = book.selectedPage; i < book.pages.size(); i ++){
                    if (book.pages.get(i).pageIcon != ItemStack.EMPTY){
                        ...
                        if (bookReqs){
                            this.renderItemStackMinusTooltipAt(book.pages.get(i).pageIcon.setStackDisplayName(I18n.format("magicpab.book."+book.name+"."+book.pages.get(i).name+".title")), basePosX+25+(ic%4)*42,basePosY+56+(ic/4)*28, mouseX, mouseY);
                            if (mouseX >= basePosX+17+(ic%4)*42 && mouseY >= basePosY+52+(ic/4)*28 && mouseX <= basePosX+49+(ic%4)*42 && mouseY <= basePosY+80+(ic/4)*28){
                                this.markTooltipForRender((new ItemStack(Items.STICK)).setStackDisplayName(I18n.format("magicpab.book."+book.name+"."+book.pages.get(i).name+".title")), mouseX, mouseY);
                            }
                        }
                        else {
                            Minecraft.getMinecraft().getTextureManager().bindTexture(new ResourceLocation("roots:textures/gui/gui_book.png"));
                            ...
                            if (mouseX >= basePosX+17+(ic%4)*42 && mouseY >= basePosY+52+(ic/4)*28 && mouseX <= basePosX+49+(ic%4)*42 && mouseY <= basePosY+80+(ic/4)*28){
                                this.markTooltipForRender((new ItemStack(Items.STICK)).setStackDisplayName(I18n.format("magicpab.book.more_knowledge")), mouseX, mouseY);
                            }
                        }
                        ic ++;
                    }
        ...
        if (page.type == com.mcmoddev.alchemytrinkets.book.page.EnumPageType.MOONLIGHT && satisfiesReqs){
        ...
        if (page.type == com.mcmoddev.alchemytrinkets.book.page.EnumPageType.RITUAL && satisfiesReqs){
        ...
        if (page.type == com.mcmoddev.alchemytrinkets.book.page.EnumPageType.CRAFTING && satisfiesReqs){
            for (int i = 0; i < 3; i ++){
                for (int j = 0; j < 3; j ++){
        ...
        if (page.type == com.mcmoddev.alchemytrinkets.book.page.EnumPageType.FURNACE && satisfiesReqs){
